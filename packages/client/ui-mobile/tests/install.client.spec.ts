...
function stubNavigator(userAgent: string, standalone?: boolean): void {
  const navigatorStub = Object.create(navigator) as Navigator & { standalone?: boolean }
  Object.defineProperty(navigatorStub, 'userAgent', { configurable: true, value: userAgent })
  if (standalone !== undefined) {
    Object.defineProperty(navigatorStub, 'standalone', { configurable: true, value: standalone })
  }
...
describe('InstallController', () => {
  ...
  it('suppresses the hint in standalone (home-screen) launch modes', () => {
    stubNavigator('Mozilla/5.0 (iPhone; CPU iPhone OS 17_0 like Mac OS X) AppleWebKit/605.1.15', true)
    const controller = new InstallController()
    controller.start()
