...
function makeCtx() {
  ...
  const byComponent = (component: unknown): unknown => {
    const factory = registrations.find(r => r.component === component)?.injectFactory
    return factory?.()
  }
...
describe('ui-mobile apply', () => {
  it('declares the layout and slots service dependencies', () => {
    expect(inject).toEqual(['layout', 'locale', 'slots', 'sessions'])
  })
  ...
  it('binds the header menu toggle to ctx.layout.toggleSidebar', () => {
    ...
    apply(ctx)
    const face = byComponent(HeaderMenuButton) as HeaderMenuButtonInjected
    face.toggleSidebar()
  ...
  it('binds the new-session menu toggle to ctx.layout.toggleSidebar', () => {
    ...
    const { ctx, layout, byComponent } = makeCtx()
    apply(ctx)
    const face = byComponent(NewSessionMenuButton) as NewSessionMenuButtonInjected
    face.toggleSidebar()
    expect(layout.toggleSidebar).toHaveBeenCalledTimes(1)
  })
  ...
  it('binds the drawer scrim to the same sidebar toggle and snapshot', () => {
    ...
    const { ctx, layout, byComponent } = makeCtx()
    apply(ctx)
    const face = byComponent(DrawerScrim) as DrawerScrimInjected
  ...
  it('binds the install inject face to the install controller', async () => {
    ...
    apply(ctx)
    const face = byComponent(InstallBanner) as InstallBannerInjected
  ...
  it('stops both controllers, the keyboard guard, and both slot contributions on teardown', () => {
    ...
    apply(ctx)
    // Locale registration, controllers, command-panel keyboard guard,
    // keyboard send action, and the page zoom lock.
