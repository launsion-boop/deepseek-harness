...
const phoneTier = MOBILE_CSS.slice(MOBILE_CSS.indexOf('@media (max-width: 639px)'))
...
describe('mobile.module.css overscroll contract', () => {
  ...
  it('docks the composer seat to the screen bottom inside the phone tier only', () => {
    ...
    expect(MOBILE_CSS.slice(0, MOBILE_CSS.indexOf('@media (max-width: 639px)'))).not.toContain('position: fixed')
  })
  ...
  it('keeps the header menu toggle icon-only and phone-tier-only', () => {
    expect(MENU_CSS).toContain('@media (min-width: 640px)')
