 ...
 * the packaged icons, and `/sw.js` serving the push-only service worker. All injection
...
/**
 * Push service worker that also removes caches left by older mobile builds.
 * @returns the complete JavaScript response body.
 */
export function buildServiceWorkerScript(): string {
  return `
const OLD_CACHE_PREFIX = 'dsh-ui-mobile-shell-';
...
function serveSw(req: IncomingMessage, res: ServerResponse): void {
...
function servePwa(req: IncomingMessage, res: ServerResponse): void {
  if (req.method !== 'GET' && req.method !== 'HEAD') {
    res.writeHead(405)
    res.end()
    return
...
function pushSubscriptionRoute(push: PushService) {
  return async (req: IncomingMessage, res: ServerResponse): Promise<void> => {
    ...
    if (req.method === 'POST') {
      ...
      if (!isPushSubscription(subscription)) { sendJson(res, 400, { error: 'invalid subscription' }); return }
...
/** Validate the persisted fields required by web-push. */
