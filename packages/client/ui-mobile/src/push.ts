...
/** Public Web Push availability and VAPID key exposed to the browser. */
...
export class PushService {
  ...
  /**
   * Check whether delivery can use a complete VAPID sender.
   * @returns whether a complete VAPID sender is configured.
   */
  enabled(): boolean { return this.publicKey !== undefined && this.privateKey !== undefined && this.subject !== undefined }

  /**
   * Project the browser-visible configuration without the private key.
   * @returns the public push configuration.
   */
  config(): PushConfig {
  ...
  /**
   * Persist or replace a browser subscription by endpoint.
   * @param subscription - validated Push API subscription.
   */
  ...
  /**
   * Remove a browser subscription when present.
   * @param endpoint - exact Push API endpoint identity.
   */
   ...
   * @param sessionId - Session opened when the notification is activated.
   */
  async notify(sessionId: string): Promise<void> {
    ...
    const expired = await Promise.all([...this.subscriptions.values()].map(async (subscription) => {
      try {
        ...
        return statusCode === 404 || statusCode === 410 ? subscription.endpoint : undefined
      }
    }))
    const endpoints = expired.filter((endpoint): endpoint is string => endpoint !== undefined)
    if (endpoints.length === 0) return
    for (const endpoint of endpoints) this.subscriptions.delete(endpoint)
    await this.save()
  }
